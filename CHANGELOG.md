...
- Release 工作流只上传用户所需产物，不再包含 electron-builder 调试配置文件。
...
- 在依赖安装后显式获取 Electron 官方运行时，保证全新开发环境和云端 Release 构建一致。
...
- 构建器只生成本地产物，GitHub 上传由单一 Release 步骤显式执行，避免 tag 环境隐式发布。
